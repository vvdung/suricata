...
static void KDD_Update_Features_TCP(ThreadVars* tv, DetectEngineCtx *de_ctx, Packet* p){
  ...
  if (direct == TOSERVER){
    ...
    if (flow->flow_state == FLOW_STATE_CLOSED) statusFlow = FLOW_STATE_CLOSED;
  ...
  // printf(" %s flow[%p][%u] hash[%u] statusFlow[%d] [%d.%d.%d.%d:%d]<->[%d.%d.%d.%d:%d] flow_state:[%hu][%hu] ssnFlags[%02X][%02X] tcpFlags[%02X][%02X] Act:%d dst[%u]src[%u]\n",
  //     czIO,flow,flow->flow_state,hash,statusFlow,
  //     ipSrc[0],ipSrc[1],ipSrc[2],ipSrc[3],portSrc,
  //     ipDst[0],ipDst[1],ipDst[2],ipDst[3],portDst,
  //     flow->flow_state,flow->use_cnt,
  //     ssn->pstate,ssn->state,
  //     prevFlags,currFlags,p->action,flow->todstpktcnt,flow->tosrcpktcnt);
